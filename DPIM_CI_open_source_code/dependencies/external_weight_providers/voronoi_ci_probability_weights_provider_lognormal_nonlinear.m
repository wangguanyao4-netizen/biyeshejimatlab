function weightData = voronoi_ci_probability_weights_provider_lognormal_nonlinear(n, d, methodName, integrandName, U, context)
persistent cacheMap
if nargin < 6
    context = struct();
end
...
Z = local_transform_to_target_space(U, opts);

Xaux = local_generate_auxiliary_samples(opts, d, context);

[labels, minDist2] = local_assign_voronoi_exact_blockwise(Xaux, Z, opts.block_size);
...
opts.center_transform = "lognormal_icdf";

opts.target_distribution = "lognormal";

opts.lognormal_mu = 0.0;

opts.lognormal_sigma = 1.0;
...
if isfield(context, "project_dir") && strlength(string(context.project_dir)) > 0
    opts.output_dir = fullfile(char(string(context.project_dir)), "ci_probability_weights_lognormal_nonlinear");
else
opts.output_dir = fullfile(pwd, "ci_probability_weights_lognormal_nonlinear");
end
...
if isfield(context, "weighting_cfg") && isstruct(context.weighting_cfg)
    ...
    if isfield(wcfg, "theta_lognormal_nonlinear_mu") && ~isempty(wcfg.theta_lognormal_nonlinear_mu)
        opts.lognormal_mu = double(wcfg.theta_lognormal_nonlinear_mu);
    end
    if isfield(wcfg, "theta_lognormal_nonlinear_sigma") && ~isempty(wcfg.theta_lognormal_nonlinear_sigma)
        opts.lognormal_sigma = double(wcfg.theta_lognormal_nonlinear_sigma);
    end
    if isfield(wcfg, "voronoi_lognormal_nonlinear_mu") && ~isempty(wcfg.voronoi_lognormal_nonlinear_mu)
        opts.lognormal_mu = double(wcfg.voronoi_lognormal_nonlinear_mu);
    end
    if isfield(wcfg, "voronoi_lognormal_nonlinear_sigma") && ~isempty(wcfg.voronoi_lognormal_nonlinear_sigma)
        opts.lognormal_sigma = double(wcfg.voronoi_lognormal_nonlinear_sigma);
    end
end
...
if ~strcmpi(string(opts.center_transform), "lognormal_icdf")
    error("The lognormal experiment requires voronoi_center_transform='lognormal_icdf'.");
end
if ~strcmpi(string(opts.target_distribution), "lognormal")
    error("The lognormal experiment requires voronoi_target_distribution='lognormal'.");
end

validateattributes(opts.lognormal_mu, {'double'}, {'scalar', 'finite'});
validateattributes(opts.lognormal_sigma, {'double'}, {'scalar', 'finite', 'positive'});
...
if isfield(context, "point_seed")
    ...
    key = sprintf('%s|n=%d|d=%d|seed=%d|aux=%d|blk=%d|tr=%s|tg=%s|lnmu=%.17g|lnsig=%.17g', ...
...
function Z = local_transform_to_target_space(U, opts)

switch lower(char(string(opts.center_transform)))
    case "lognormal_icdf"
epsProb = 1e-15;
        Uc = min(max(U, epsProb), 1 - epsProb);

standardNormal = sqrt(2) * erfinv(2 * Uc - 1);
Z = exp(opts.lognormal_mu + opts.lognormal_sigma * standardNormal);
...
Z = U;
    ...
    otherwise
        error("Unsupported center transform '%s'.", opts.center_transform);
end
end
...
switch lower(char(string(opts.target_distribution)))
    case "lognormal"
        Xaux = exp(opts.lognormal_mu + opts.lognormal_sigma * randn(opts.aux_sample_count, d));
