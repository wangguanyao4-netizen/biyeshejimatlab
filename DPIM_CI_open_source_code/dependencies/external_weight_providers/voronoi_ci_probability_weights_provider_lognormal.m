function weightData = voronoi_ci_probability_weights_provider_lognormal(n, d, methodName, integrandName, U, context)
persistent cacheMap
if nargin < 6
    context = struct();
end
...
if isfield(context, "project_dir") && strlength(string(context.project_dir)) > 0
    opts.output_dir = fullfile(char(string(context.project_dir)), "ci_probability_weights_lognormal");
else
opts.output_dir = fullfile(pwd, "ci_probability_weights_lognormal");
end
...
if isfield(context, "weighting_cfg") && isstruct(context.weighting_cfg)
    ...
    if isfield(wcfg, "theta_lognormal_mu") && ~isempty(wcfg.theta_lognormal_mu)
        opts.lognormal_mu = double(wcfg.theta_lognormal_mu);
    end
    if isfield(wcfg, "theta_lognormal_sigma") && ~isempty(wcfg.theta_lognormal_sigma)
        opts.lognormal_sigma = double(wcfg.theta_lognormal_sigma);
    end
    if isfield(wcfg, "voronoi_lognormal_mu") && ~isempty(wcfg.voronoi_lognormal_mu)
        opts.lognormal_mu = double(wcfg.voronoi_lognormal_mu);
    end
    if isfield(wcfg, "voronoi_lognormal_sigma") && ~isempty(wcfg.voronoi_lognormal_sigma)
        opts.lognormal_sigma = double(wcfg.voronoi_lognormal_sigma);
    end
end
