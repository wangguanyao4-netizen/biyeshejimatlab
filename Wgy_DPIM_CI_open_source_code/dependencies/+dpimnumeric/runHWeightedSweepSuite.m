...
manifest.note = "WEIGHTED version: E3/E4/E5 use Voronoi probability weights (pool-based, standard-normal theta). E1/E2/E7/E8 use original equal-weight KDE.";
...
base = localBaseConfig(runMode);
poolCount = base.pool_size;
config = struct( ...
    "experiment", "E3_linear_beam_n_h_weighted", ...
    "alpha", 0.05, ...
    "lambda", 5, ...
    "d", 10, ...
    "x0", 0.5, ...
...
for iH = 1:numel(config.h_list)
    ...
    for iN = 1:numel(config.n_list)
        ...
        fprintf("    Building weighted pool: h=%.4g n=%d pool=%d\n", h, nInner, poolCount);
        pool = localBuildWeightedEstimatePool(poolCount, nInner, config.d, coeffs, config.y_target, h, config.seed + iH + nInner);
        for iR = 1:numel(config.R_list)
            R = config.R_list(iR);
            acc = localEmptyAccumulator(localScalarMethods());
            trialDetails = localEmptyScalarTrialDetails(config.M, R);
            for trial = 1:config.M
                estimates = pool(randi(poolCount, R, 1));
                ciSeed = config.seed + 1000 * trial + iH + R;
                ci = localCiMethods(estimates, config.alpha, config.B, ciSeed, truth);
                ci = localAppendFallbackRule(ci, config.lambda);
                acc = localAccumulate(acc, ci, truth);
            ...
            detailBlockIndex = detailBlockIndex + 1;
            detailBlocks{detailBlockIndex, 1} = localPackScalarDetailBlock( ...
                struct("experiment", "E3_linear_beam_n_h_weighted", "h_index", iH, "h", h, ...
...
localWriteSummary(outDir, summary, config, "E3 linear beam WEIGHTED h/fallback sweep (Voronoi probability weights).");
localSaveDetailBlocks(outDir, detailBlocks, config);
localPlotCoverageByH(outDir, summary, "E3 weighted coverage by h", "E3_hfallback_weighted_coverage.png");
result = localResultPaths(outDir);
end
...
for iCase = 1:numel(cases)
    ...
    responseTruth = localLockedNormTruth(cfg.d, cfg.truth_sample_count, cfg.seed + 17);
    ...
    for iH = 1:numel(hList)
        ...
        for iN = 1:numel(cfg.n_list)
            ...
            for iy = 1:numel(yValues)
                ...
                for trial = 1:cfg.M
                    estimates = localNonlinearKernelEstimatesNorm(cfg.R, nInner, cfg.d, yTarget, h);
                ...
                detailBlockIndex = detailBlockIndex + 1;
                detailBlocks{detailBlockIndex, 1} = localPackScalarDetailBlock( ...
                    struct("experiment", "E4_nonlinear_tail_weighted", "case_id", string(cfg.case_id), ...
...
config = struct("experiment", "E4_nonlinear_tail_weighted", "run_mode", char(runMode), ...
    "cases", cases, "note", "E4: standard-normal theta (randn), equal-weight KDE (on-the-fly, no pool).");
localWriteSummary(outDir, summary, config, "E4 nonlinear normal-theta h/fallback sweep.");
localSaveDetailBlocks(outDir, detailBlocks, config);
...
base = localBaseConfig(runMode);
targetDimension = 5;
responseTruth = localLockedNormTruth(targetDimension, base.truth_sample_count, 20260604 + 51);
    ...
    for iH = 1:numel(hList)
        ...
        for iN = 1:numel(base.e5_n_list)
            ...
            for iy = 1:numel(yValues)
                ...
                for trial = 1:base.M
                    estimates = localNonlinearKernelEstimatesNorm(base.e5_R, nInner, targetDimension, yTarget, h);
                ...
                detailBlockIndex = detailBlockIndex + 1;
                detailBlocks{detailBlockIndex, 1} = localPackScalarDetailBlock( ...
                    struct("experiment", "E5_bootstrap_t_instability_weighted", "case_id", string(cfg.case_id), ...
...
summary = movevars(summary, ["case_id", "d", "h_index", "h", "n", "y_index", "y_value", ...
    "truth_smoothed_density"], "Before", 1);
config = struct("experiment", "E5_bootstrap_t_instability_weighted", "run_mode", char(runMode), ...
    "d", targetDimension, "lambda", base.lambda, "cases", cases, ...
    "note", "E5: standard-normal theta (randn), equal-weight KDE (on-the-fly).");
localWriteSummary(outDir, summary, config, "E5 targeted nonlinear normal-theta h/fallback sweep.");
localSaveDetailBlocks(outDir, detailBlocks, config);
...
function response = localLockedNormTruth(d, sampleCount, seed)
previousState = rng;
rng(seed, "twister");
cleanupObj = onCleanup(@() rng(previousState)); %#ok<NASGU>
theta = randn(sampleCount, d);
response = localLockedResponse(theta);
end
...
function estimates = localNonlinearKernelEstimatesNorm(R, nInner, d, yTarget, h)
estimates = zeros(R, 1);
for r = 1:R
    theta = randn(nInner, d);
    response = localLockedResponse(theta);
...
function pool = localBuildWeightedEstimatePool(poolSize, nInner, d, coeffs, yTarget, h, baseSeed)
% Build pool of probability-weighted kernel density estimates
% Formula: p̂(y) = Σ w_i K_h(y - g(θ_i)), w_i from resolve_estimator_weights
% Uses resolve_estimator_weights (same pattern as run_single_experiment.m:371)
persistent weightCache
if isempty(weightCache)
    weightCache = containers.Map('KeyType', 'char', 'ValueType', 'any');
end
pool = zeros(poolSize, 1);
useProvider = nInner <= 128;
for idx = 1:poolSize
    seed = baseSeed + idx;
    rng(seed, 'twister');
    if useProvider
        U = rand(nInner, d);
        theta = norminv(max(min(U, 1-1e-15), 1e-15));
        response = theta * coeffs;
        kv = localNormalPdf(yTarget, response, h);
        cacheKey = sprintf('V_%d_%d_%d', nInner, d, mod(seed, 100));
        if isKey(weightCache, cacheKey)
            w = weightCache(cacheKey);
        else
            wInfo = resolve_estimator_weights(struct('mode','probability','source','voronoi_ci_probability_weights_provider','normalize',true), ...
                nInner, d, 'mc', 'density', U, struct());
            w = wInfo.weights;
            if numel(weightCache) < 10
                weightCache(cacheKey) = w;
            end
        end
    else
        theta = randn(nInner, d);
